...
a critical step to search improvement.  For many search algorithms this may
be trivial, but in Cartesian Genetic Programming (CGP), and Genetic Programming (GP)
in general, complex genotype to phenotype map makes achieving this understanding a
challenge.  By examining tuned parameter values, variant success on different
problem difficulties, node behavior, and offspring replacement properties we
seek to comprehend observed characteristics of CGP search.  We focus our study
around methods to prevent wasted CGP evaluations (\emph{Skip}, \emph{Accumulate},
and \emph{Single}) and methods to overcome CGP's search limitations imposed by
genome ordering (\emph{Reorder} and \emph{DAG}).

We find strong evidence that currently most CGP techniques evolve genomes that
are highly inactive, very redundant, and full of seemingly useless constants.
In some instances we found less than 1\% of the genome was actually required to
encode the evolved solution.
Without \emph{Reorder} or \emph{DAG} almost the entire latter portion of the genome
is never used by any ancestor of the evolved solution.  More generally, our
results lead us to conclude that \emph{Skip Reorder} and \emph{Single Reorder}
are most likely to solve hard problems in the least number of evaluations and
the least amount of time, respectively, with the least amount of degenerate
behavior.
...
mutating node, giving it a complexity of $O(AN)$.  In the worst case, this algorithm
...
mutation is $O(mA^2N^2)$, as when $m$ is the mutation rate we expect $mAN$
...
and therefore we expect it to be added sooner.  The greater the discrepancy in these
...
\label{sec:quality}
While it is possible to theorize about how the effects of each CGP variant on search and
...
\label{sec:parameter}
To perform a fair comparison, we should ensure proper parameter configuration.
...
problem, we can look for general trends.  From the table, it appears on easier problems
...
The semantics of a node is a complete description of its functionality.
Any two nodes with identical semantic values have identical behavior,
