...
limited-cardinality relationships between variables. Such problems are
well-known and exist in many real world problems (network security, computational biology, VLSI design, and statistical physics). These
problems are amenable to local search using a Hamming-Ball Hill Climber
(HBHC) that can efficiently find r-bit local optimal. We show how a
simple memetic algorithm in conjunction with HBHC can find \emph{global}
optima for gray-box problems. Finally a gray-box version of P3, which
utilizes the information about variable relationships given by the gray-box
problem itself, is the most efficient of the examined algorithms.
