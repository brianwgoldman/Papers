...
complete subfunctions between individual has a high likelihood of preserving quality.
Each cluster created contains at least one entire subfunction, and two
...
each $x_i$'s subfunction depends on: Nearest Neighbor NKq and Unrestricted NKq.
In Nearest Neighbor NKq each variable depends on the $K$ which immediately follow
it in the genome, with variables at the end of the solution wrapping to depend on the
starting variables. Landscapes of this form can be solved in polynomial
...
and that both $c$ and $k$ do not increase as $N$ increases, ensuring that the efficiency
...
is the global optimum, verified using dynamic programming.
