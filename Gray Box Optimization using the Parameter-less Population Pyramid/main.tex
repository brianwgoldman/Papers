...
On NKq this results in a x375 speedup for problems with at least 1000 variables.
...
\begin{figure*}
  ...
  \caption{Comparison of how radius effects solution quality at termination. For NKq-Landscapes $N=6000$ and $K=4$ and
...
\begin{figure*}
  ...
  \caption{Comparison of solution quality during optimization on a log-log scale for different algorithms. For NKq-Landscapes $N=6000$ and $K=4$ and
  for Ising Spin Glasses $N=6084$. Each algorithm uses its best found $r$ value.}
...
\begin{figure*}
  ...
  For NKq-Landscapes $N=6000$ and $K=4$ and for Ising Spin Glasses $N=6084$.}
...
varying $N$ from 200 to 10,000 for NKq and 196 to 6084 for Ising Spin Glass.
...
$N=4096$ and 21 runs of $N=6084$. The median run of the HBHC was unable to reach the best fitness
...
was able to consistently reach the best fitness until it it hit the memory limit on $N=2000$ for
NKq and $N=2916$ for Ising Spin Glass. This is because unlike the other methods, Black-Box P3
...
methods. Applying regression to both lines, we estimate that Black-Box P3's time to global optimum
...
4.6 times slower than Gray-Box on $N=2025$. The regression line suggests that while
...
with TUX occasionally performing better. When $N>2000$, Gray-Box P3 finds better quality
...
on problems with even moderate epistasis. Furthermore, in general is obtains almost no improvement
...
uniform crossover potentially allows for the HBHC to find unrelated improvements.

While TUX improves over plain HBHC, Gray-Box P3 is required to perform truly successful
global optimization. Gray-Box P3 replaces na\"{i}ve local search the HBHC and utilizes
...
would be to explore methods of performing efficient learning on top of the known variable iterations.
...
even moderately high $r$ values hit our memory limit, and may be causing Black-Box P3 to have
difficulty learning clusters.
