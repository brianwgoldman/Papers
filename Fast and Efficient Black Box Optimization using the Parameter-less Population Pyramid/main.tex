...
evolutionary optimization without requiring problem specific user specified parameters.
...
than all tested competitors. P3 also in general maintains the highest fitness reached when comparing
algorithms based on different number of evaluations performed. Some evidence suggests this performance
also extends to considering total number seconds to find the global optimum.
...
at each level growing as search progresses. P3 uses a model based method learns a linkage tree from gene epistasis as a method for
performing effective crossover. Combined with a simple hill climber, P3's design contains
