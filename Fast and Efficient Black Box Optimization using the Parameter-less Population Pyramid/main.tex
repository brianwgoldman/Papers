...
evolutionary optimization without requiring problem specific parameters.
...
generally finds the highest quality solutions. Some evidence also suggests this performance
extends to considering total number seconds to find the global optimum even when using efficient fitness functions.
...
previous parameter-less techniques P3 is more efficient than existing parameterized methods for
...
Our implementations of all of these algorithms as well as all of the raw results and processing scripts
...
configurations. hBOA, LTGA, and Parameter-less hBOA only outperform P3 on the 5, 4, and 1 smallest Deceptive
Step Trap instances, respectively. Random Restart Hill Climbing outperforms P3 on the 3 smallest Nearest Neighbor NK instances
and the smallest Ising Spin Glass. $1+(\lambda, \lambda)$ has the most success outperforming P3, doing so
on 5 smallest Deceptive Traps, 3 smallest Deceptive Step Traps, and 2 smallest Rastrigin.
...
At 63\% of recording intervals hBOA has the worst fitness of any solver. Many of the exception occur when
the total number of evaluations performed is significantly less than the genome size and hBOA reaches better fitness
