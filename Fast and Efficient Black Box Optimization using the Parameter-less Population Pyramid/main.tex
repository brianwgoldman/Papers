...
as $O(\mu)$ donations per evaluation, but experimental evidence suggest that this
operation actually saves more time than it costs by increasing the number of evaluations
per model rebuild.
...
upward. This allowed for continuous integration of diversity as the lowest population is reseeded
...
doing was provably less efficient than using an optimal population size.
...
the population. These solutions are then used to build the model used to generate $\mu$ new
...
subset of genes are set to these values, how frequently is gene x set to value v?'' This can
be represented using a decision forest, with each tree in the forest representing one gene
...
with each leaf storing a pointer to all selected solutions. Each iteration the algorithm considers
...
$x_j = 1$ move to the other. Each possible split is scored using Equation~\ref{eq-hboa} with the best
split kept. This process continues until none of the splits result in a higher quality. To favor compact
models, Equation~\ref{eq-hboa} is scaled by Equation~\ref{eq-hboa-stop}, which provides increased cost
for more total leaves.

\begin{equation}
  BDe(B) = p(B)\prod_{i=0}^{N}\prod_{l\in L_i} \frac{\Gamma(m'_i(l))}{\Gamma(m_i(l) + m'_i(l))}
...
binary alphabets. As the $\Gamma(n) = (n-1)!$ this means the top term in the middle product and the bottom
...
satisfy this inequality are considered enough to keep. Furthermore, whichever split maximizes the right side of the
