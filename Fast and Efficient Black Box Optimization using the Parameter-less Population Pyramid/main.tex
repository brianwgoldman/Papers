...
For Deceptive Trap and Deceptive Step Trap using 4 or less traps, $1+(\lambda, \lambda)$ performs significantly
...
For larger problems it becomes increasingly unlikely that $1+(\lambda, \lambda)$'s mutation and crossover can
create enough changes to the correct parts of an individual required to overcome deception without negatively changing others. This
behavior also causes high variance in success rate, as evident by the occasional successes on large Deceptive Trap problems.

By integrating local search, P3 gains most of the benefits. Unlike the other model based methods, P3 generally outperforms
...
Only techniques which explicitly built models of the search landscape were able to solve the largest problem
...
the single instance problems can all be perfectly represented with a linkage tree while none of the randomly generated
problem classes can.
...
LTGA and hBOA on large problem sizes. P3 solves its median run of Nearest Neighbor
NK and Ising Spin Glasses faster than both hBOA and LTGA.
...
best of all techniques on both Trap functions, HIFF, Ising Spin Glass, and MAX-SAT problems. A likely
