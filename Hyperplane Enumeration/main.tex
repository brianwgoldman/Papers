...
In this subset, the graph is defined as a square two-dimensional toroidal grid with each edge
...
Performing the swap improves the minimum dependency of $O_{\hat{i}+1}$ more than it worsens
...
A $k$-bound separable problem any problem that is composed of non-overlapping
...
all variables which appear in the same $s_i$ are consecutively placed in enumeration.
Consider a numbering of $f_i$ such that if $i < j$ then all of $s_i$'s
...
contains an improving move. In total, enumerating $f_i$ requires less than $2^k$
steps. Each $f_i$ is considered sequentially until none contains an improving move,
meaning the first local optimum is found in \BigO{2^kN/k} time.
...
2-bit local optima. Here we extend those conclusions for up to $N=59$ and up to $r=4$. Adjacent
NKq tend to have more 1-bit local optima than Random NKq, most of which disappear as $r$
...
and with $N=78$ Adjacent NKq still only has fewer than 500 million.
...
relationship generally does not change as $r$ increases. Even for $r=4$ the general
number and growth rate of local optima appears to stay the same for both problems.
...
In our original experiment we tested problems up to size $N=100$, which was sufficient to
...
in both the runtime and number of local optima. 18 of 30 runs completed in under 1 minute,
while the slowest run took 1.5 hours. 7 had less than one thousand 4-bit local optima, while 5
had over one hundred thousand.
