\chapter{Gray-Box P3}
%wfp comment. First use of acronym HBHC. introduce it.
...
the direct non-linear relationships between variables are known. As a
result, %wfp comma
%wfp 
Black-box 
P3 no longer needs to store the pairwise frequency information 
% wfp that leadsthe black-box version to require 
%wfp
requiring \BigO{N^2} memory. Instead,
...
The cluster splitting process has identical properties 
% wfp as 
%wfp
to 
...
% wfp possible due to the gray-box domain 
is that each time a donation is made, only the effected part of the solution's fitness needs to be
recalculated. As a result, %wfp comma
the number of subfunction evaluations required
