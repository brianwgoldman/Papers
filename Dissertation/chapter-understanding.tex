...
with those difficulties. Similarly, knowing which characteristics favor
each existing algorithm can help researchers choose the algorithm
...
distance from the nearest global optimum and fitness difference from the global optima.
...
$X_i$ is a fitness improvement. If flipping $X_i$ in a given solution is a fitness improvement
then any solution that contains the same values of $X_i$ and its $ck$ dependencies cannot be a local optima.

This knowledge can be effectively exploited to skip parts of the enumeration.
For each fitness-improving move, we can determine the least significant bit it depends on.
