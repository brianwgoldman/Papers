...
\begin{figure*}
  ...
  \caption{Terminology diagram for finding all local optima. $X_i$ is a bit flip
           with dependencies on $index$ 2, 4, 5, and 7, giving it a minimum dependency
           of 2. After reordering, its minimum dependency becomes 4.}
  \label{fig-enumerate}
\end{figure*}
...
\begin{figure}
  ...
  \Procedure{AllLocalOptima}{}
    ...
    \While{$i < N$}
      \State $i \leftarrow $\Call{FindNextIndex}{$i$}
      \If{$i = -1$}
        \State $found \leftarrow found + [solution]$
        \State $i \leftarrow 0$
      \EndIf
      \While{$i < N$ \textbf{and} $solution[i] = 1$}
        \State \Call{MakeMove}{$i$}
    ...
    \State \Return $found$
  \EndProcedure
  \Procedure{FindNextIndex}{$i$}
    \While{$i \ge 0$}
      \ForAll{$m \in move\_bin[i]$}
        \If{$delta[m] \ge 0$}
          \Return $i$
        \EndIf
      \EndFor
...
For gray-box problems there are only $ck$ other bit values that 
need to be checked to determine if
% bwg: put "flipping" back in as X_i itself is just a bit, not a flip.
flipping $X_i$ is a fitness improvement. If flipping $X_i$ in a given solution is a fitness improvement
then any solution that contains the
original
values of $X_i$ and its $ck$ dependencies cannot be a local optima.
...
%wfp comment. What does "minimum" mean here. Is it literally the least
%significant bit of a subfunction?
