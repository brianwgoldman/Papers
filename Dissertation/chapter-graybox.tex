...
% wfp All the algorithm has to perform search
...
% wfp have no expectation of quality. 
As such,
...
to the same value. As a result, %wfp comma
...
% wfp comment. Somewhere you should define subfunction. It could be
% the elements of an NK but you really mean it more broadly than
% that. For example, you refer to "clause" on MAX-SAT, subfunction on
% NK, but then only continue to talk about subfunction.
...
respect to
%wfp For best efficiency $c$ should
% be constant in regards to 
...
subfunctions in which a variable appears is set to 12.81. %wfp where does this come from?
Assuming that all subfunctions use $k$ variables and each variable appears in $c$ subfunctions
provides a bound on the total number of subfunctions %wfp of
$\frac{cN}{k}$.
...
tested moves must be tested again. As a result, %wfp comma
...
moves that overlap that subfunction. Combined, %wfp comma
...
search heuristics the less greedy, %wfp commad
...
increases. However, %wfp commad
in the gray-box
domain not all combinations need to be tested. Consider that if two variables do not participate
in the same subfunction, the relationship between their effects is, % wfp comma
by definition,  % wfp comma
additive. As such
...
there is only an edge if the two variables have a direct, %wfp comma
...
%wfp comment. Hamming ball, first use here. Should be defined (or
%somehow described, referenced, something.
...
that must be tested to determine if a solution is an $r$-bit local optima is \BigO{N}. This means that
...
of the tournament. Iterative construction is possible because, %wfp comma
when a sub-bracket is complete, %wfp comma
...
it allows for learning from previous local optima and, %wfp comma
as Section~\ref{chapter-gray-box-benchmarking} will show, %wfp comma
