...
\begin{abstract}
%Maximum 200 Words
  A major limiting factor to practical application of evolutionary
  techniques is the tuning of their parameters which need to be
  reconfigured to each new problem.  While some previous methods been
  able to remove or reduce the need for parameter tuning, many do so
  ...
  requiring \textbf{no} parameters and still broadly applicable to
  many problem types. It strikes a balance between continuous
  ...
  parameters, P3 has a lower growth complexity when test problems come
  ...
  are indicative wide applicability.
\end{abstract}
...
assumptions about the nature of the problem is is solving as no
